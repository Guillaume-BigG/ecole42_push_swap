...
/*   Updated: 2024/03/25 17:30:37 by gbeaudoi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_strjoinfree_bonus(char *s1, char *s2)
{
	char	*str;
	size_t	i;
	size_t	j;
...
char	**ft_argv_to_one_line_bonus(char **argv, int argc, int i)
{
	...
	while (argv[++i])
	{
		bigline = ft_strjoinfree_bonus(bigline, argv[i]);
		if (bigline == NULL)
			exit(1);
		if (i != (argc - 1))
		{
			bigline = ft_strjoinfree_bonus(bigline, " ");
			if (bigline == NULL)
				exit(1);
		}
	}
